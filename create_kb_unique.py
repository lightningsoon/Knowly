...
from llama_index.core import StorageContext,load_index_from_storage,Settings,load_indices_from_storage
from llama_index.embeddings.dashscope import (
    DashScopeEmbedding,
    DashScopeTextEmbeddingModels,
    DashScopeTextEmbeddingType,
...
def query(prompt,db_name,similarity_threshold=0.35,chunk_cnt=20):
    ...
    for i in range(len(results)):
        if results[i].score >= similarity_threshold:
            print(results[i].score,similarity_threshold)
            chunk_text = chunk_text + f"## {i+1}:\n {results[i].text}\n"
            chunk_show = chunk_show + f"## {i+1}:\n {results[i].text[:10].replace('\n','')}\nscore: {round(results[i].score,2)}\n"
    print(f"已获取chunk：{chunk_show}")
    return chunk_text