...
@app.post("/query")
async def query_endpoint(prompt,db_name,similarity_threshold=0.35,chunk_cnt=20):
# def query_endpoint(request:str):
    # print(request)
    # similarity_threshold=0.35
#     return ""
    # prompt,db_name=request.split('|')
    if isinstance(similarity_threshold,str):
        similarity_threshold=float(similarity_threshold)/100
    # result = query(prompt, db_name, similarity_threshold, chunk_cnt)
    result = query(prompt, db_name)
    return result
# files_info = list_files_in_index("说明书")
# for file in files_info:
#     print(f"ID: {file['id']}, Text: {file['text'][:10]}, Metadata: {file['metadata']}")
#     print("-"*10)
